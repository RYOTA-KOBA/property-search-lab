...
- リポジトリは**必ず private** にする。公開リポジトリにしない
  (※ このリポジトリは既存の public リポジトリを流用しており、上記の通りユーザー承認のもと例外的に public のまま運用している)
...
- [x] `docs/results.md` を作成し、V1〜V7 の結果を記録する
- [x] 各シナリオについて「合格 / 不合格 / 保留」と、判断に至った観測値を書く
- [x] 最後に @docs/verification-plan.md の「最終的に出したい結論」の4問に答える

**全タスク完了(2026-09-06)**。V1(表記ゆれ正規化のみ保留)、V2〜V6合格、V7は参考値を記録。
結論は @docs/results.md を参照。
